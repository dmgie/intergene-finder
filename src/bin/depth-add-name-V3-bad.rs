...
It also assumes that both the bed file and the depth file are sorted by start position / base position respectively",
    ...
    if matches.value_of("depth").unwrap().len() > 1 {
        ...
        for i in depthfiles {
            ...
            add_name_to_depths(depths.as_mut(), &bed_regions);
        ...
        add_name_to_depths(&mut depths, &bed_regions);
...
fn add_name_to_depths(depths: &mut Vec<DepthInfo>, bed_regions: &Vec<BedRegion>) {
    // Adds the name of the region to the depth file, based on the bed file

    let mut depth_index = 0;
    let mut prev_depth_index = 0;
    let mut bed_idx = 0;

    // Take the amount of elements depending on length of current bed index
    // https://stackoverflow.com/questions/71718210/what-is-the-idiomatic-way-to-assign-one-value-to-a-slice-in-rust
    while depth_index < depths.len() {
        let slice_length = bed_regions[bed_idx].length;
        let to_name = &bed_regions[bed_idx].name;

        depth_index += slice_length as usize;
        depths[prev_depth_index..depth_index]
            .iter_mut()
            .for_each(|d| d.name = to_name.to_string());
        prev_depth_index = slice_length as usize + 1;
        bed_idx += 1
    }
}
